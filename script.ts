...
document.addEventListener("DOMContentLoaded", function () {
  ...
  interface ContentBlock {
    type: ContentType;
    content?: string;
    spans?: SpanContent[]; // NEW: Supports spans inside text & list items
    items?: { text: string; spans?: SpanContent[] }[]; // NEW: Each list item supports spans
    listTitle?: string;
    listTitleStyle?: Record<string, string>; // NEW: Separate styling for list titles
    listItemStyle?: Record<string, string>; // NEW: Separate styling for list items
    listGap?: string; // NEW: Defines spacing between list items
    style?: Record<string, string>;
    buttonText?: string;
    position?: number;
  }
  ...
  if (itemId && contentData[itemId]) {
    ...
    sortedBlocks.forEach((block) => {
      ...
      if (block.type === "image" && block.content) {
        ...
        if (block.style) {
          Object.keys(block.style).forEach((key) => {
            img.style.setProperty(key, block.style![key]);
          });
        }
        ...
        // Handle spans
        if (block.spans) {
          block.spans.forEach((spanData) => {
            const span = document.createElement("span");
            span.textContent = spanData.text;
            if (spanData.style) {
              Object.keys(spanData.style).forEach((key) => {
        ...
        if (block.listTitle) {
          ...
          if (block.listTitleStyle) {
            Object.keys(block.listTitleStyle).forEach((key) => {
              title.style.setProperty(key, block.listTitleStyle![key]);
            });
          }
        ...
        block.items.forEach((itemData) => {
          ...
          if (block.listItemStyle) {
            Object.keys(block.listItemStyle).forEach((key) => {
          ...
          if (itemData.spans) {
            itemData.spans.forEach((spanData) => {
              const span = document.createElement("span");
              span.textContent = spanData.text;
              if (spanData.style) {
                Object.keys(spanData.style).forEach((key) => {
        ...
        // Apply list gap
        if (block.listGap) {
          listElement.style.gap = block.listGap;
